...
export default function Reception() {
  ...
  const arrivalsCount = mockReservations.filter(r => r.status === 'Confirmée').length;
  const departuresCount = mockReservations.filter(r => r.status === 'En séjour').length; // simulated for demo
  ...
  const updateRoomStatus = (roomId: string, newStatus: TRoomStatus) => {
    setRooms(rooms.map(r => r.id === roomId ? { ...r, current_status: newStatus } : r));
    if (selectedRoom && selectedRoom.id === roomId) {
      setSelectedRoom({ ...selectedRoom, current_status: newStatus });
    }
    setSuccessMsg(`Statut de la chambre ${rooms.find(r => r.id === roomId)?.room_number} mis à jour : ${newStatus}`);
  ...
  const getGuestForRoom = (roomNum: string) => {
    ...
    if (res) {
      return mockGuests.find(g => g.id === res.guest_id);
    }
    return null;
  };

  const getReservationForRoom = (roomId: string) => {
    return mockReservations.find(r => r.room_id === roomId && r.status === 'En séjour');
  };
                          ...
                          return (
                            <tr key={res.id} className="hover:bg-slate-50/50">
                              ...
                              <td className="py-3.5 text-right">
                                <button 
                                  onClick={() => {
                                    updateRoomStatus(res.room_id, 'Occupée');
                          ...
                          return (
                            <tr key={res.id} className="hover:bg-slate-50/50">
                              ...
                              <td className="py-3.5 text-right">
                                <button 
                                  onClick={() => {
                                    updateRoomStatus(res.room_id, 'À nettoyer');
                                    res.status = 'Terminée';
                                  }}
