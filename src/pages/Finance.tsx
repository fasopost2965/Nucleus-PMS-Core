...
import { mockInvoices, mockPayments, mockGuests } from '../mockData';
import { IInvoice, IPayment } from '../types';
...
export default function Finance() {
  ...
  const handleAddExpense = (e: React.FormEvent) => {
    ...
    setSuccessMsg(`Dépense de ${expAmt.toLocaleString()} XOF enregistrée.`);
    setTimeout(() => setSuccessMsg(''), 4000);
  };

  const getGuestName = (guestId: string) => {
    const g = mockGuests.find(guest => guest.id === guestId);
  ...
  const registerInvoicePayment = (invId: string) => {
    ...
    setSuccessMsg(`Encaissement de ${payAmt.toLocaleString()} XOF validé pour la facture ${inv.invoice_number}.`);
    setTimeout(() => setSuccessMsg(''), 4000);
  };
                  ...
                  {invoices
                    ...
                    .map((inv) => (
                      <tr key={inv.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="py-3.5 px-4 text-right font-semibold font-mono text-slate-600">{(inv.subtotal).toLocaleString()} XOF</td>
                        <td className="py-3.5 px-4 text-right font-medium font-mono text-slate-500">{(inv.tax).toLocaleString()} XOF</td>
                        <td className="py-3.5 px-4 text-right font-extrabold font-mono text-slate-900">{(inv.total).toLocaleString()} XOF</td>
                        <td className="py-3.5 px-4 text-right font-bold font-mono text-rose-600">{(inv.balance).toLocaleString()} XOF</td>
                        ...
                        <td className="py-3.5 px-6 text-right space-x-1">
                          ...
                          <button
                            onClick={() => alert('Impression du reçu PDF simulé.')}
                  ...
                  {payments
                    ...
                    .map((pay) => (
                      <tr key={pay.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="py-3.5 px-4 text-right font-black font-mono text-emerald-600">{(pay.amount).toLocaleString()} XOF</td>
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {expenses.map((e) => (
                      <tr key={e.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="py-3 px-4 text-right font-black font-mono text-rose-600">{(e.amount).toLocaleString()} XOF</td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="p-5 border border-slate-200 rounded-xl space-y-3 bg-slate-50/20">
                  ...
                  <div className="space-y-1.5 text-slate-600 font-semibold">
                    <p className="flex justify-between"><span>Solde d'ouverture (08:00) :</span> <span className="font-mono text-slate-800">150 000 XOF</span></p>
                    <p className="flex justify-between text-emerald-600"><span>Encaissements espèces (+) :</span> <span className="font-mono">+ 50 000 XOF</span></p>
                    <p className="flex justify-between text-rose-600"><span>Dépenses espèces (-) :</span> <span className="font-mono">- 37 500 XOF</span></p>
                    <div className="h-px bg-slate-200 my-1"></div>
                    <p className="flex justify-between font-bold text-sm text-slate-900"><span>Solde théorique actuel :</span> <span className="font-mono">162 500 XOF</span></p>
                  </div>
                </div>
