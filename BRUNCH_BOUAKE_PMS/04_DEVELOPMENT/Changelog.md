...
## [1.3.2] - 2026-07-19
### Correctifs critiques — robustesse de connexion MySQL et fuite d'identifiants
* **Aucune reconnexion automatique à MySQL** : dès qu'une erreur de connexion faisait passer `isMySQLOnline` à `false` (`server/config/db.ts`), rien ne retentait jamais la connexion — le processus restait sur le fallback JSON local pour toute sa durée de vie, même après le retour de MySQL, jusqu'à un redémarrage manuel de l'application ou un appel à la route admin `/admin/db-diagnostics` (elle-même inaccessible sans être déjà connecté). Un simple incident réseau transitoire pouvait donc bloquer silencieusement et durablement un déploiement sur le JSON, rendant invisible toute correction faite directement en base (ex. un mot de passe modifié via phpMyAdmin) tant que l'app n'était pas redémarrée manuellement. **Corrigé** : `ensureMySQLOnline()` retente automatiquement la connexion (avec un cooldown de 15s) à chaque lecture/écriture, dans `getCollection`, `saveCollection`, `runTransaction`, `inspectUsersSchema`, `backup` et `restore`.
* **Identifiants réels en clair dans le bundle JS client** (`src/pages/Login.tsx`) : un tableau `ALLOWED_USERS` codé en dur contenait les emails et le mot de passe en clair (`Prodesk@2026`) de plusieurs comptes réels, dont deux Super Administrateur — visible par quiconque inspecte le bundle JavaScript livré au navigateur, sur n'importe quel déploiement. Ce fallback s'activait dès que l'appel à l'API échouait autrement qu'avec un rejet explicite d'identifiants (ex. erreur réseau/serveur). **Supprimé entièrement** — l'API backend est désormais la seule source d'authentification, un échec de connexion réseau affiche un message d'erreur générique au lieu de retomber sur des identifiants codés en dur.
* **Mot de passe stocké en clair dans `localStorage`** ("Se souvenir de moi") : en plus de l'email, le mot de passe saisi était persisté tel quel dans le stockage local du navigateur. **Corrigé** : seul l'email est désormais mémorisé.
